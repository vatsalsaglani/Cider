...
**Proposed linked TODO workflow:** [TODOs, individual chats, notes, graph and CLI/skill](../plan/05-linked-todos.md) now specifies TODO detail, explicit many-to-many chat/note links, note backlinks, response-to-note actions and a bounded context bundle. The user-requested graph adds local/workspace views over those same relationships, exact entity navigation and filters. The plan includes current storage gaps, migration, capture-before-pruning, scoped Stop history and read-only CLI/skill delivery. Planning only; no task migration, linking/graph feature, CLI or skill installation implemented.
