...
**Linked TODO worktree plans:** the [nine-plan implementation set](../plans/linked-work/00-overview.md) divides the [TODO/chat/note/graph proposal](../plan/05-linked-todos.md) into a local contract phase, parallel store/TODO/notes lanes, local integration, parallel journal/graph/CLI-skill lanes, then final packaging and verification. Each plan has exact ownership, source evidence, checks and an agent start prompt. The existing app is now recorded at source baseline `192b50f`; its 53 Swift tests passed. Planning only: no feature implementation, migration, worktree agent dispatch or skill installation. Start with plan 01 locally; parallel agents require its compiled, committed contracts first.
