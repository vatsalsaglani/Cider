...
> Read `docs/plans/linked-work/00-overview.md` and `docs/plans/linked-work/01-contracts.md`, plus `.agents/skills/working-with-cider/SKILL.md`. Implement plan 01 on `linked-work/01-contracts`. Run locally as the coordinator; do not create a lane worktree for this plan. Goal: Create the compiled shared types, SQLite schema, protocol fakes and package seams that all later lanes will consume. Edit only the files in this plan's File ownership list, including its own Deviations section; keep all frozen contracts and sibling files unchanged. Follow the overview's data, hook, isolation and local-commit rules. Run these verification commands from the repo root: `swift build --product Cider; swift build --product cider; swift build --product cider-events; swift test; git diff --check`. Also complete the plan's explicit integration/manual gates when applicable; never claim unrun checks passed. Commit locally without pushing. Finish with what works, base/head IDs, changed files, each verification result, merge risks, and Deviations (or state none).
