...
This is the specification for the local contract phase, not a claim that these APIs already exist. Plan 01 must turn it into compiled Swift declarations, schema and fixtures, resolve every unspecified initializer/default in those files, then freeze the exact commit before worktrees start. A required correction is recorded here by the coordinator before that freeze. Later lanes may change implementation bodies only in their owned files; they never change these shared declarations.
